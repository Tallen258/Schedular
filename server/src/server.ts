...
/**
 * POST /api/google/calendar/sync - Import Google Calendar events to local database
 */
...
/**
 * POST /api/google/calendar/import/:eventId - Import a single Google Calendar event
 */
app.post("/api/google/calendar/import/:eventId", async (req: Request, res: Response) => {
  try {
    if (!req.user?.email) return res.status(401).send("Login required");
